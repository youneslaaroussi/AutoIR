...
export default class Daemon extends Command {
	...
	private async startAlertingLoop(flags: any, opts?: {demo?: boolean}): Promise<void> {
		if (opts?.demo) {
			await this.runDemoAlertLoop(flags)
			return
		}
		// Resolve TiDB DSN
						...
						if (channels.includes('slack')) {
							try {
								...
								if (slackCfg?.botToken && slackCfg.channelId) {
									await sendSlackBotMessage(slackCfg.botToken, slackCfg.channelId, `*${title}*\n${body}`)
								} else if (slackWebhook) {
									await postSlack(slackWebhook, title, body)
								}
							} catch (e) { this.log(`[alerts] Slack error: ${String((e as any)?.message || e)}`) }
						}
	...
	private async runDemoAlertLoop(flags: any): Promise<void> {
		...
		const sendSlack = async (title: string, body: string) => {
			if (!channels.has('slack')) return
			try {
				if (slackCfg?.botToken && slackCfg.channelId) {
					await sendSlackBotMessage(slackCfg.botToken, slackCfg.channelId, `*${title}*\n${body}`)
				} else if (slackWebhook) {
		...
		const loop = async () => {
			while (true) {
				...
				const sev = demoPick(['medium','high','critical'], [0.4, 0.4, 0.2])
				const conf = Math.round(70 + Math.random()*30)
				const title = `[${sev.toUpperCase()}][${conf}%] ${demoPick([
					'Latency spike detected in payment service',
					'Increasing 5xx errors in API gateway',
					'SQS queue backlog growing rapidly',
					'High error rate in auth service',
					'Deadlock patterns observed in TiDB cluster'
				])}`
				const body = [
					`Time: ${now}`,
					'Top groups: /aws/lambda/payments(523), /ecs/api(312), /rds/aurora(201)',
					'Samples: 5',
					'Suggested actions:',
					'- Scale out API tasks by +2',
					'- Roll back payments service to previous version',
					'- Increase DB connection pool size',
				].join('\n')
				await sendSlack(title, body)

				if (tickCount % demoMajorEvery === 0) {
					const rptTitle = ':trophy: Weekly Reliability Report — AutoIR'
					const rptBody = [
						'*Executive summary*:\nUptime 99.98%. Two critical incidents auto-mitigated. MTTD 45s, MTTR 6m.',
						'*Top issues*:\n1) API 5xx spikes during deploy\n2) Payment latency due to cold starts',
						'*Recommendations*:\n- Adopt blue/green for payments\n- Enable provisioned concurrency for hot paths',
					].join('\n\n')
					await sendSlack(rptTitle, rptBody)
				}
...
function demoPick<T>(arr: T[], weights?: number[]): T {
